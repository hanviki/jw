...
if test -f ./env/${RUNTIME_MARK}/env.cfg ; then
    ...
    exportFile ./env/${RUNTIME_MARK}/env.cfg
else
    echo -e "./env/${RUNTIME_MARK}/env.cfg is not exist"
fi

if test -f ./env/${RUNTIME_MARK}/${BRANCH_MARK}/env.cfg ; then
    # export $(sed '/^#/d' ./env/${RUNTIME_MARK}/${BRANCH_MARK}/env.cfg)
    exportFile "./env/${RUNTIME_MARK}/${BRANCH_MARK}/env.cfg"
else
    echo -e "./env/${RUNTIME_MARK}/${BRANCH_MARK}/env.cfg is not exist"
fi
...
if test "${DEPLOYMENT_EXEC}"x == "install"x ; then
    ...
    cat ./tmp/${YAML_TARGET_FILENAME}.yml
